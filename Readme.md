...
2025/9/28 - 改了下，只要是游戏中都可以Pause，Pause后中止inputManager的自定义行为(不阻止菜单的鼠标键)，Fixed：向下小箭头出现偏移，加了一个EmoteBubble的效果

TODO: 交互时出现光标

TODO: 打算加一个文字出来的时候稍微抖一下的效果(不知道需不需要)，出现暂停菜单的时候需要允许语言刷新(没写)

TODO: map美术需要重新设置front

TODO: EventBus防止重新订阅存在问题

TODO: timeline(没写完)，任务系统(还没写)

TODO: 暂停菜单允许/禁止全屏，调整分辨率

操作指示：现在你可以通过WASD或者方向键进行移动，按E(交互)键进行和物体的交互，ESC打开设置菜单进行游戏设定/键盘操作的调整，按下H(提示)键获得推进游戏剧情的提示

告示牌：一块老旧的告示牌，上面写着基本移动操作WASD，按下鼠标左键或者对话键进行和物体的交互，靠近可以交互的物品会有对话气泡提示，普通的物体会显示出眼睛图标，可以推进剧情并且未探索过的物体会出现"..."提示，按住菜单键打开菜单进行游戏设置和按键操作的调整，按下H(提示)键获得推进游戏剧情的提示

H键：总之，先到处走走看看吧……

靠近关键的物品会有对话提示，普通的是眼睛图标……按下(对话键)进行交互

按经验，只要把所有关键线索都排查完，就能推进了

所有基础操作应该都写在身后的告示牌上了，如果突然想不起来了，可以过去确认一下
